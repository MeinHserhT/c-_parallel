...
// Direction kernel
__global__ void calculateDirectionKernel(int * importance, 
		int width, int height)
{
	...
	if (c < width)
	{
		extern __shared__ int s_importance[];

		int cur_c= threadIdx.x + 1;
		// Load lowest line data to SMEM
		s_importance[cur_c] = importance[(height - 1) * width + c];
		...
		{
			int i = r*width + c;
			int left = (c == 0) ? 44444444 : s_importance[1];
			int right = (c == width - 1) ? 44444444 : s_importance[blockDim.x];
			int mini = min(min(s_importance[cur_c], right), left);
			
			// wait for other threads to finish 
			// save to global memory
			int energy = importance[i] + mini;
			importance[i] = energy;
			__syncthreads();

			s_importance[cur_c] = energy;
			__syncthreads();
		}
  }
}
...
// Allocate device memories
	...
	size_t imageSize = width * height, filterWidth = 3,
			shared_Row = blockSize.x * sizeof(int);
...
// input reduce pixels 
	...
	for (int loop = 0; loop < pixelsReduce; loop++)
	{
		...
		//TODO-> Direction
		calculateDirectionKernel<<<int(grid.x), int(blockSize.x), shared_Row >>>(d_importance, width, height);
...
// Device -------
  dim3 blockSize(32, 32); // Default
	...
	// using kernel 1
	seamCarvingbyDevice(inPixels, width, height, grayPixels1, outPixels1, importance1, filter_x_sobel, filter_y_sobel, pixelsReduce, 2, blockSize);
